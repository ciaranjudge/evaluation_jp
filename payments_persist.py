...
@timeit
def data_to_hdf(data, store_filepath, key, overwrite=True):
    # Create or overwrite the store if this is the first file to be processed
    print(str(store_filepath))
    # if overwrite:
...
for key, filepath in key__source_filepaths.items():
    ...
    data = load_sas(filepath, rows=1000)
    ...
    if overwrite:
        ...
        data_to_hdf(data, store_filepath, key, overwrite)
        # print("Create master dataframe for all periods")
        # all_data = data
        # overwrite = False
    else:
        print("Save the cleaned dataset to HDF5")
        data_to_hdf(data, store_filepath, key)
